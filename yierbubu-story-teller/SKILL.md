...
description: 专门用于构思和编写“一二布布”（治愈系Q版熊情侣）的绘本剧情和故事脚本。仅限剧情创作，不包含提示词生成。
---
...
- **The Corgi** (if present) MUST be a **Yellow/White Pembroke Welsh Corgi (Q版柯基)**.
...
- **Script Only**: Do not generate AIGC prompts. If the user asks for prompts, suggest using the `yierbubu-prompt-writer` skill.
